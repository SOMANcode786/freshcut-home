...
for (const patch of [{items:[]},{items:'bad'},{customer:{}},{customer:{name:' ',phone:'1',address:'a'}},{items:[{id:1,weight:'250g',qty:-1}]},{items:[{id:1,weight:'250g',qty:1.5}]},{items:[{id:1,weight:'invalid_weight',qty:1}]},{items:[{id:99,weight:'250g',qty:1}]},{items:[{id:2,weight:'250g',qty:1}]}]) test(`invalid order ${JSON.stringify(patch)}`,async () => assert.equal((await request('/api/orders',{method:'POST',body:{...orderBody(),...patch}})).status,400));
